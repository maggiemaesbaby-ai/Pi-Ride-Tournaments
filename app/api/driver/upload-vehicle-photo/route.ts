...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: updateError } = await supabase
      .from("drivers")
      .update({ vehicle_photo_url: photoBase64 })
      .eq("pi_user_id", piUserId)

    if (updateError) {
      console.error("[v0] Failed to update vehicle photo:", updateError)
      return NextResponse.json({ error: "Failed to upload photo" }, { status: 500 })
    }

    console.log(`[v0] Driver ${driver.full_name} vehicle photo uploaded successfully`)
